 ...
 * (see CreateVPCEndpoint in cbdinocluster's awscontrol package) — the Capella-side
 * cluster UUID, *not* cbdinocluster's own tracking id (the one used for
 * connstr/remove). We look the endpoint up by that tag rather than needing the
 * endpoint id from setup-link's output.
 *
 * Run on its own:
 *   bun src/cloud/util/aws/delete-vpc-endpoints.ts --cloud-cluster-id <capella-cluster-uuid>
 */
...
/**
 * Delete any VPC endpoint(s) tagged with the given Capella cloud cluster id (see
 ...
 */
export async function deleteVpcEndpointsForCluster(cloudClusterId: string): Promise<void> {
  const described = await ec2Client.send(
    new DescribeVpcEndpointsCommand({ Filters: [{ Name: "tag:Cbdc2ClusterId", Values: [cloudClusterId] }] }),
  );
  ...
  console.log(`  deleting VPC endpoint(s) for cloud cluster ${cloudClusterId}: ${endpointIds.join(", ")}`);
...
if (isMain(import.meta.url)) {
  runCli(async () => {
    ...
    const idIndex = argv.indexOf("--cloud-cluster-id");
    const cloudClusterId = idIndex !== -1 ? argv[idIndex + 1] : undefined;
    if (!cloudClusterId) {
      throw new Error("Usage: delete-vpc-endpoints.ts --cloud-cluster-id <capella-cluster-uuid>");
    }
    await prepareAwsCli();
    logAwsAction("Deleting VPC endpoint(s)", { cloudClusterId });
