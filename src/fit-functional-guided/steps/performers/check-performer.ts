...
/** Absolute path to an SDK's performer on disk. */
export function performerPath(sdk: Sdk): string {
  if (sdk.jvm) {
    return join(repoPath(JVM_CLIENTS), `${sdk.performer}-fit-performer`);
  }
...
if (isMain(import.meta.url)) {
  ...
  runCli(async () => {
    ...
    if (!sdk) {
      ...
      console.error(
        `Usage: tsx src/fit-functional-guided/steps/performers/check-performer.ts <${values}>`,
      );
      process.exit(2);
    }
